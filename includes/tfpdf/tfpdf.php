...
class tFPDF {
	...
	protected $column_start_x;     // column start x position
	protected $column_end_x;       // column end x position
	protected $column_width;	   // column width
	 ...
	 * Maybe add page
	 */
	function maybe_add_page( $orientation = '', $size = '', $rotation = 0 ) {
		$next_page = $this->PageNo() + 1;

		if ( empty( $this->pages[ $next_page ] ) ) {
	...
	function AddFont( $family, $style = '', $file = '', $uni = false ) {
		...
		if ( $uni ) {
			if ( defined( '_SYSTEM_TTFONTS' ) && file_exists( _SYSTEM_TTFONTS . $file ) ) {
				$ttffilename = _SYSTEM_TTFONTS . $file; } else {
	...
	function Cell( $w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = false, $link = '' ) {
		...
		if ( $this->y + $h > $this->PageBreakTrigger && ! $this->InHeader && ! $this->InFooter && $this->AcceptPageBreak() ) {
			...
			$this->maybe_add_page( $this->CurOrientation, $this->CurPageSize, $this->CurRotation );
	...
	function Write( $h, $txt, $link = '' ) {
		...
		$room_left = $this->column_end_x - $this->GetX();
	...
	protected function _putlinks( $n ) {
		foreach ( $this->PageLinks[ $n ] as $pl ) {
			...
			$s    = '<</Type /Annot /Subtype /Link /Rect [' . $rect . '] /F 4 /Border [0 0 0] ';
