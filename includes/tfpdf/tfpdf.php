...
class tFPDF {
	...
	function Cell( $w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = false, $link = '' ) {
		...
		if ( $txt !== '' ) {
			...
			if ( $this->ws && $this->unifontSubset ) {
				...
				// ASDASDADS
				// $s = sprintf( 'BT /H1 <</MCID 0 >> BDC %.2F %.2F Td %s Tj EMC ET', $x * $this->k, ( $this->h - $y ) * $this->k, $txt2 );

				// $s = 'BT /H1 <</MCID 0 >> BDC ';
				$s .= sprintf( 'BT %.2F %.2F Td %s Tj ET', ( $this->x + $dx ) * $k, ( $this->h - ( $this->y + .5 * $h + .3 * $this->FontSize ) ) * $k, $txt2 );
				// $s .= sprintf( 'BT %.2F %.2F Td %s Tj EMC ET', ( $this->x + $dx ) * $k, ( $this->h - ( $this->y + .5 * $h + .3 * $this->FontSize ) ) * $k, $txt2 );
			}
	...
	function test() {
		...
		$this->_newobj();
		$this->_put( '<<' );
		$this->_put( '/Type /StructTreeRoot' );
		$this->_put( '/K [ 3 0 R ]' );
		// $this->_put( '/ParentTree 2 0 R' );
		$this->_put( '>>' );
		$this->_put( 'endobj' );

		// Put H1 structure
		$this->_newobj();
		$this->_put( '<</Type /StructElem' );
		$this->_put( '/S /H1' );
		$this->_put( '/P 2 0 R' );
		$this->_put( '/K [0]' );
		$this->_put( '/Pg 1 0 R' );
	...
	protected function _putcatalog() {
		...
		$this->_put( '/StructTreeRoot 3 0 R' );
