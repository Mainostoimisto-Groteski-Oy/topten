jQuery(document).ready(($) => {
	if( $('body').hasClass('single-tulkintakortti') || $('body').hasClass('single-ohjekortti') || $('body').hasClass('single-lomakekortti') ) {
		if ( $('aside.sidebar div.box').length > 0 ) {
			$('aside.sidebar div.box .box-title').on('click', function() {
				...
				$(this).children('button').attr('aria-expanded', (i, attr) => attr === 'true' ? 'false' : 'true');
				// toggle this parent child box-content aria-expanded attribute
				$(this).parent().children('.box-content').attr('aria-expanded', (i, attr) => attr === 'true' ? 'false' : 'true');
			});
