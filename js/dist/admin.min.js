...
jQuery(document).ready(function ($) {
  ...
  function cardSearch() {
    var cardStatusType = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'valid';
...
jQuery(document).ready(function ($) {
  function getChild(node) {
    ...
    if (tag === 'input' || tag === 'textarea') {
      nodeData.tag = 'input';
      var id = node.id;
      var labelText = $("label[for=\"".concat(id, "\"] .label-text")).text();
      nodeData.attributes = {
        type: node.type,
        value: node.value,
  ...
  function generatePDFdata(parent) {
    ...
    rows.each(function (rowIndex, row) {
      ...
      $(row).find('.column-item').each(function (columnIndex, column) {
        rowData.count += 1;
    ...
    $.ajax({
      ...
      if (response.success) {
        for (var index in response.data) {
          $("#".concat(index)).val(response.data[index]);
        }
      }
    }).fail(function (jqXHR, textStatus, errorThrown) {
      // eslint-disable-next-line no-console
