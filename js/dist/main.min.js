...
jQuery(document).ready(function ($) {
  ...
  $('.sidebar-navigation li.page_item_has_children > a').after('<button class="nav-button"><span class="material-symbols-outlined nav-icon">add</span></button>');
  ...
  function setNavbarHeight() {
    var navbar = $('.site-header').height();
    $(':root').css('--navbar-height', "".concat(navbar, "px"));
  }
...
document.addEventListener('DOMContentLoaded', function () {
  var nodelist = document.querySelectorAll('.animate-on-scroll');
  var elements = (0,_babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_0__["default"])(nodelist);
  function triggerAnimation() {
    elements = elements.filter(function (el) {
      ...
      if (top + top / 5 < window.innerHeight) {
        var animationName = element.dataset.animation;
...
function initSliders() {
  var sliders = document.querySelectorAll('.splide.carousel');
...
(() => {
/*!*******************************************!*\
  !*** ./js/src/cards/card-notification.js ***!
  \*******************************************/
jQuery(document).ready(function ($) {
  if ($('section.cards-notification').length > 0) {
    var breadcrumbs = $('section.page-breadcrumbs').outerHeight();
    ...
    var combinedHeight = breadcrumbs + wpadminbar;
    var topValue = masthead + wpadminbar;
...
jQuery(document).ready(function ($) {
  ...
  if ($('#primary').attr('data-template')) {
    var cardStatusType = $('#primary').attr('data-template');
  ...
  function cardSearch() {
    ...
    if (localStorage.getItem('cardCategory')) {
      ...
      var splitCategory = cardCategory.split('|');
    ...
    var cardkeywords = JSON.parse(localStorage.getItem('keywords'));
    ...
    $.ajax({
      ...
      data: {
        ...
        cardDateStart: cardDateStart,
        cardDateEnd: cardDateEnd,
      ...
      success: function success(data) {
        ...
        $('#tulkintakortit ul.children').each(function () {
          var tulkintaStatus = [];
          var grandparent = $(this).attr('data-parent');
          $(this).children().children('ul.grandchildren').each(function () {
            var parent = $(this).attr('data-parent');
            if ($(this).children().length <= 0) {
              $(this).parent("li.child[data-id=".concat(parent, "]")).hide();
          ...
          if (tulkintaStatus.indexOf('true') === -1) {
            $("#tulkintakortit li.parent[data-id=".concat(grandparent, "]")).hide();
          }
        });
  ...
  function updateFilters(type) {
    ...
    var words = JSON.parse(localStorage.getItem("".concat(type)));
    var typeName = '';
    ...
    if (words) {
      $.ajax({
        ...
        success: function success(data) {
          var terms = data.data;
          if (terms.length > 0) {
            $("#selected".concat(type)).html('');
            $("#selected".concat(type)).parent('figure').addClass('active');
            $(terms).each(function () {
              $("#selected".concat(type)).append("<li class=\"keyword\" data-id=\"".concat(this.value, "\"><button class=\"removekeyword\" data-type=\"").concat(type, "\" data-id=\"").concat(this.value, "\"></button><span>").concat(this.label, "</span></li>"));
            });
          }
        },
        error: function error(errorThrown) {
  ...
  function showFilters(type) {
    ...
    // Get items from local storage
    if (type === 'cardLaw') {
      var cardLaw = localStorage.getItem('cardLaw');
      if (cardLaw) {
        // localstorage doesn't take array values so we need to split the string
        var splitLaw = cardLaw.split('|');
        $('#selectedLaw').html('');
        $("#selectedLaw").parent('figure').addClass('active');
        $('#selectedLaw').append("<li class=\"keyword\" data-id=\"".concat(splitLaw[0], "\"><button class=\"removekeyword\" data-type=\"cardLaw\" data-id=\"").concat(splitLaw[0], "\"></button><span>").concat(splitLaw[1], "</span></li>"));
      }
    }
    if (type === 'cardCategory') {
      var cardCategory = localStorage.getItem('cardCategory');
      if (cardCategory) {
        var splitCategory = cardCategory.split('|');
        $('#selectedCategory').html('');
        $("#selectedCategory").parent('figure').addClass('active');
        $('#selectedCategory').append("<li class=\"keyword\" data-id=\"".concat(splitCategory[0], "\"><button class=\"removekeyword\" data-type=\"cardCategory\" data-id=\"").concat(splitCategory[0], "\"></button><span>").concat(splitCategory[1], "</span></li>"));
      }
    }
    if (type === 'cardDateStart') {
      var cardDateStart = localStorage.getItem('cardDateStart');
      if (cardDateStart) {
        var date = new Date(cardDateStart);
        ...
        $('#selectedDateStart').append("<li class=\"keyword\" data-id=\"dateStart\"><button class=\"removekeyword\" data-type=\"cardDateStart\" data-time=\"".concat(cardDateStart, "\" data-id=\"dateStart\"></button><span>").concat(dateString, "</span></li>"));
      }
    }
    if (type === 'cardDateEnd') {
      var cardDateEnd = localStorage.getItem('cardDateEnd');
      if (cardDateEnd) {
        $('#selectedDateEnd').html('');
        $("#selectedDateEnd").parent('ul').parent('figure').addClass('active');
        var _date = new Date(cardDateEnd);
        var _day = _date.getDate();
        var _month = _date.getMonth() + 1;
        var _year = _date.getFullYear();
        var _dateString = "".concat(_day, ".").concat(_month, ".").concat(_year);
        $('#selectedDateEnd').append("<li class=\"keyword\" data-id=\"dateEnd\"><button class=\"removekeyword\" data-type=\"cardDateEnd\" data-time=\"".concat(cardDateEnd, "\" data-id=\"dateEnd\"></button><span>").concat(_dateString, "</span></li>"));
      }
    }
    if (type === 'freeText') {
      var freeText = localStorage.getItem('freeText');
      if (freeText) {
        $('#selectedFreeText').html('');
        $("#selectedFreeText").parent('figure').addClass('active');
        $('#selectedFreeText').append("<li class=\"keyword\" data-id=\"freeText\"><button class=\"removekeyword\" data-type=\"freeText\" data-id=\"freeText\"></button><span>".concat(freeText, "</span></li>"));
      }
    }
  }
  ...
  function applyFilters(type) {
    ...
    var keyword = '';
    if (type === 'keywords') {
      keyword = $("#card".concat(type, "Value")).val();
      ...
      if (localStorage.getItem(type)) {
        ...
        localStorage.setItem(type, JSON.stringify(_storage));
      }
      // Update filters to DOM
      if ($("#selected".concat(type)).children().length > 0) {
        var existingChildren = [];
        // Check if item is already in DOM
        $("#selected".concat(type)).children().each(function () {
          existingChildren.push($(this).attr('data-id'));
        });
        // If not, append it
        if (existingChildren.indexOf(keyword) === -1) {
          $("#selected".concat(type)).parent('figure').addClass('active');
          $("#selected".concat(type)).append("<li class=\"keyword\" data-id=\"".concat(keyword, "\"><button class=\"removekeyword\" data-type=\"").concat(type, "\" data-id=\"").concat(keyword, "\"></button><span>").concat($("#card".concat(type)).val(), "</span></li>"));
        }
      } else {
      ...
      $("input[name=\"cardclassfilter\"]:checked").each(function () {
        cardClasses.push("".concat($(this).val(), "|").concat($(this).attr('data-name')));
      });
      localStorage.setItem(type, JSON.stringify(cardClasses));
      // Update filters to DOM
      if ($('#selectedCardClasses').length > 0) {
        var _storage2 = JSON.parse(localStorage.getItem(type));
        $('#selectedCardClasses').html('');
        $('#selectedCardClasses').parent('figure').addClass('active');
        $(_storage2).each(function () {
          // split the string into an array
          var split = this.toString().split('|');
          $('#selectedCardClasses').append("<li class=\"keyword\" data-id=\"".concat(split[0], "\"><button class=\"removekeyword\" data-type=\"cardclassfilter\" data-id=\"").concat(split[0], "\"></button><span>").concat(split[1], "</span></li>"));
        });
      }
    }
  }
  function removeFilters(id, type) {
    if (type === 'keywords') {
      ...
      $(storage).each(function () {
        ...
        if (storedItem === id) {
          ...
          if (storage.length <= 0) {
            ...
            $("li.keyword[data-id=".concat(id, "]")).parents('figure').removeClass('active');
          } else {
            localStorage.setItem(type, JSON.stringify(storage));
          }
        }
      ...
      $("li.keyword[data-id=\"".concat(id, "\"]")).remove();
    } else if (type === 'cardLaw') {
      var _storage3 = localStorage.getItem(type);
      var splitLaw = _storage3.split('|');
      if (splitLaw[0] === id) {
        localStorage.removeItem(type);
        $("li.keyword[data-id=".concat(id, "]")).parents('figure').removeClass('active');
        $("li.keyword[data-id=".concat(id, "]")).remove();
      ...
      var _storage4 = localStorage.getItem(type);
      var splitCategory = _storage4.split('|');
      ...
      var _storage5 = JSON.parse(localStorage.getItem(type));
      // We want at least one to be selected and shown at all times
      if (_storage5.length > 1) {
        $("ul#selectedCardClasses li.keyword").removeClass('disabled');
        $(_storage5).each(function () {
          var storedItem = this.toString().split('|')[0];
          if (storedItem === id) {
            // Remove this id from the array
            _storage5.splice(_storage5.indexOf(id), 1);
            // Push back to localstorage

            localStorage.setItem(type, JSON.stringify(_storage5));
        ...
        if (_storage5.length > 1) {
          ...
          $("ul#selectedCardClasses li.keyword[data-id=\"".concat(id, "\"]")).addClass('disabled');
        }
      } else {
        $("ul#selectedCardClasses li.keyword[data-id=\"".concat(id, "\"]")).addClass('disabled');
      }
  ...
  function autoCompleteField(type, minLen) {
    ...
    $("#card".concat(type)).autocomplete({
      source: function source(request, response) {
        var userInput = $("#card".concat(type)).val();
        $.ajax({
          url: Ajax.url,
          method: 'POST',
          data: {
      ...
      select: function select(e, ui) {
        $("#card".concat(type)).val(ui.item.label);
        $("#card".concat(type, "Value")).val(ui.item.value);
        return false;
      },
      minLength: minLen
      // Show the list of values
    })._renderItem = function (ul, item) {
      return $('<li></li>').data('item.autocomplete', item.label).append("<div data-id=\"".concat(item.value, "\">").concat(item.label, "</div>")).appendTo(ul);
    };
  }
  ...
  function cardTypeFilters(id, name) {
    // These exist
    if ($("input[name=".concat(name, "]")).length > 0) {
      ...
      if (name === 'cardclassfilter') {
        if ($("input[name=".concat(name, "]:checked")).length > 0) {
          ...
          $("input[name=".concat(name, "][value=").concat(id, "]")).prop('checked', true);
        }
  ...
  if ($('#searchCards').length > 0) {
    ...
    if ($('#classCheckboxes').length > 0) {
      ...
      localStorage.setItem('cardclassfilter', JSON.stringify(_checkedBoxes));
    }
    var cardClassFilterLength = $('#classCheckboxes input').length;
    ...
    $('#searchCards button.searchTrigger').on('click', function () {
      ...
      if ($('#cardLaw').val() !== '') {
        var cardLawID = $('#cardLaw').val();
        var cardLawName = $('#cardLaw').find(':selected').attr('data-name');
        var toStore = "".concat(cardLawID, "|").concat(cardLawName);
        localStorage.setItem('cardLaw', toStore);
      }
      if ($('#cardCategory').val() !== '') {
        var cardCategoryID = $('#cardCategory').val();
        var cardCategoryName = $('#cardCategory').find(':selected').attr('data-name');
        var _toStore = "".concat(cardCategoryID, "|").concat(cardCategoryName);
        localStorage.setItem('cardCategory', _toStore);
      }
      var checkedBoxes = [];
      $('#classCheckboxes input:checked').each(function () {
        checkedBoxes.push($(this).val());
    ...
    $('#cardkeywords').on('keypress', function (e) {
      if (e.which === 13) {
        applyFilters('keywords');
      }
    });
...
jQuery(document).ready(function ($) {
  ...
  function generatePDFdata(parent) {
    ...
    var rows = parent.children('.row-block');
    rows.each(function (rowIndex, row) {
      ...
      $(row).find('.column').each(function (columnIndex, column) {
        rowData.count += 1;
        var width = getComputedStyle(column).getPropertyValue('--width');
        var columnData = {
          data: [],
          attributes: {
        ...
        $(column).children().each(function (childIndex, child) {
  ...
  $('.save-as-pdf').on('click', function () {
    ...
    var cardContent = $("article.".concat(type, " .card-content"));

    // Get card content
    var data = generatePDFdata(cardContent);
    console.log(data);
    $.ajax({
      url: Ajax.url,
      type: 'POST',
...
jQuery(document).ready(function ($) {
  if ($('body').hasClass('single-tulkintakortti') || $('body').hasClass('single-ohjekortti') || $('body').hasClass('single-lomakekortti')) {
    if ($('aside.sidebar div.box').length > 0) {
      $('aside.sidebar div.box .box-title').on('click', function () {
        ...
        $(this).parent().children('.box-content').attr('aria-expanded', function (i, attr) {
          return attr === 'true' ? 'false' : 'true';
        });
      });
  ...
  $('#sidebar-menu .keywords .keyword a.keyword-link').on('mouseenter', function () {
    ...
    var height = $('.keyword-description-container[id=desc-' + id + ']').height() + 35;
    console.log(height);
    // get left border of ul.keywords
    var left = $('#sidebar-menu .keywords').offset().left;
    // get left border of keyword
    var keywordLeft = $(this).offset().left;
    // get width of keyword
    var keywordWidth = $(this).parent('.keyword').innerWidth();
    // calculate left offset for keyword-description-container
    var leftOffset = keywordLeft - left - keywordWidth;
    if (leftOffset < 0) {
...
!function (e, n, A) {
  ...
  function i(e, n) {
    if ("object" == (0,_babel_runtime_helpers_typeof__WEBPACK_IMPORTED_MODULE_0__["default"])(e)) for (var A in e) f(e, A) && i(A, e[A]);else {
      e = e.toLowerCase();
      var o = e.split("."),
        t = Modernizr[o[0]];
...
"use strict";
/*!*****************************************!*\
  !*** ./js/src/variables/breakpoints.js ***!
  \*****************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
